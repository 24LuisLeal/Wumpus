...
public class Game {
    ...
    public static void colocateNpcInRandomPosition(){
        ...
        myPlayer = new Player(Setting.getSetting().getOxygenOfPlayer(),myMaze[0]);
        available.remove(0);
        //System.out.println("++++++++");
    ...
    public static void interactWithPlayer(){
        ...
        for (int i = 0; i < Setting.getSetting().getBat(); i++) {
            ...
            if(myBat[i].getPosition().getId() == myPlayer.getPosition().getId()){
                //Entonces la posición del jugador cambia
                int sendToFly = (int)(Math.random() * Setting.getSetting().getCavern());
